...
/// @title Rust/Go/TypeScript/Solidity dock manifest equality gate（preimage v2）
/// @notice Reads the canonical manifest generated by uvp-core
///         (gen_dock_fixtures) and recomputes every EVM word encoding consumed
///         by UVPDockingModule v2: 定义引用、端口叶/接口叶、绑定哈希、双根、
///         new/existing 双模式 routeHash、实例/子单身份、envelope 幂等键、
///         permit digest。This prevents the Solidity side from silently
///         retaining a copied or stale vector.
contract DockManifestParityTest {
    ...
    function _manifest() private view returns (string memory) {
        return vm.readFile(string.concat(vm.projectRoot(), "/../../../uvp-core/fixtures/dock/v1/manifest.json"));
    }
